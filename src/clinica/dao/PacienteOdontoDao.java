 ...
 * @author celso
 */
public class PacienteOdontoDao {

    Connection conn;

    public PacienteOdontoDao() {
    ...
    public boolean verificaCpf(String cpf) {
        ...
        try {
            String sql = "SELECT * FROM tblPacienteOdonto WHERE pacCpf = ?";
            ...
            Logger.getLogger(PacienteOdontoDao.class.getName()).log(Level.SEVERE, null, ex);
            return false;
        } finally {
            try {
                conn.close();
    ...
    public List<Paciente> buscaPacId(String id) {
        try {
            String sql = "SELECT * FROM tblPacienteOdonto WHERE pacID = ? ORDER BY pacID";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, id);
            ResultSet rs = stmt.executeQuery();
    ...
    public List<Paciente> buscaPacCpf(String cpf) {
        try {
            String sql = "SELECT * FROM tblPacienteOdonto WHERE pacCpf = ? ORDER BY pacCpf";
            PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    public List<Paciente> buscaPacNome(String str) {
        try {
            String sql = "SELECT * FROM tblPacienteOdonto WHERE pacNome LIKE ? ORDER BY pacNome";
            PreparedStatement stmt = conn.prepareStatement(sql);
            ...
            //String Sql = "SELECT * FROM tblPacienteOdonto WHERE ? LIKE ?";
            List<Paciente> pacEncontrados = new ArrayList<Paciente>();
    ...
    public int ExcluirPaciente(Paciente p1) {
        try {
            String sql = "DELETE FROM tblPacienteOdonto WHERE pacID = ?";
            PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    public int AdicionarPaciente(Paciente p1) {
        try {
            String sql = "INSERT INTO tblPacienteOdonto VALUES(null, null,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            PreparedStatement stmt = conn.prepareStatement(sql);
            ...
            return LastID("tblPacienteOdonto");
    ...
    public int AlterarPaciente(Paciente p1) {
        try {
            String sql = "UPDATE tblPacienteOdonto set pacNome = ?, pacCpf = ?, pacDeclara = ?, pacSexo = ?, pacNasc = ?, pacEndereco = ?, pacNum = ?, pacComplemento = ?, pacBairro = ?, pacCidade = ?, pacUF = ?, pacCep = ?,"
    ...
    public int BuscaNovoID() {
        try {
            ...
            String sql = "SELECT count(*) FROM tblPacienteOdonto";
            PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    private int FisrtID(String tbl) {
        try {
            String sql = "SELECT pacID FROM tblPacienteOdonto LIMIT 1";
            PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    public int LastID(String Tbl) {
        try {
            String sql = "SELECT pacID FROM tblPacienteOdonto ORDER by pacID DESC LIMIT 1";
            PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    public Paciente BuscaPaciente(int id) {
        try {
            String sql = "SELECT * FROM tblPacienteOdonto WHERE pacID = ?";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setLong(1, id);
    ...
    public Paciente PrimeiroRegistro() {
        return BuscaPaciente(FisrtID("tblPacienteOdonto"));
    }
    
    public Paciente UltimoRegistro(){    
        return BuscaPaciente(LastID("tblPacienteOdonto"));
    }
    
    public Paciente ProximoRegistro(int id) {
        try {
            String sql = "SELECT pacID FROM tblPacienteOdonto WHERE pacID > ? LIMIT 1";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setInt(1, id);
            ResultSet rs = stmt.executeQuery();
    ...
    public Paciente RegistroAnterior(int id) {
        try {
            String sql = "SELECT pacID FROM tblPacienteOdonto WHERE pacID < ? ORDER by pacID DESC LIMIT 1";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setInt(1, id);
            ResultSet rs = stmt.executeQuery();
