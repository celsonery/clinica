    ...
    public String getEndereco(String CEP) throws IOException {
        try {
            Document doc = Jsoup.connect("http://www.qualocep.com/busca-cep" + CEP)
                    .timeout(120000)
                    .get();
            Elements urlPesquisa = doc.select("span[itemprop=streetAdreess]");
            for (Element urlEndereco : urlPesquisa) {
                return urlEndereco.text();
            }
        } catch (SocketTimeoutException ex) {
            System.out.println("Erro timout: " + ex.getMessage());
        } catch (HttpStatusException wx) {
            System.out.println("Erro Http: " + wx.getMessage());
        }
        return CEP;
    }
